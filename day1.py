...
inp = list(filter(lambda x: len(x)>0,"""
gsjgklneight6zqfz
    ...
    for char in line:
        ...
        for i,word in enumerate(numbers):
            if buf.endswith(word):
                buf = buf[0:len(buf)-len(word)]
                buf = f"{buf}{9-i}{word[-1]}"
    proc.append(buf)

print(proc)

nums = [re.sub("[^0-9]","",x) for x in proc]

accu = []
for x in nums:
    accu.append(f"{x[0]}{x[-1]}")
print(accu)
...
data = zip(inp,proc,nums,accu)
print([len(list(x)) for x in [inp,proc,nums,accu]])
with open("dump.csv", 'w', newline='') as file:
    writer = csv.writer(file)
    
    # Write header if needed
    writer.writerow(['input', 'processed', 'numerical', 'final'])  # Adjust column names as needed
    for row in data:
        print(row)
        a,b,c,d = row
